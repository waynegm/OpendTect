...
HorizonSectionOsgCallBack::HorizonSectionOsgCallBack( HorizonSection* hrsection )
...
{ 
	...
	if( ( hrsection_->forceupdate_ || eyeChanged( projectiondirection ) ||
	    initialtimes_<cMinInitialTimes ) ) // during initialization, we update two times,first time is lowest resolution, second is current resolution
	{
